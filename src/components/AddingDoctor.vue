        ...
        <div class="modal-body" v-if="selectedDoctor">
          <div class="view-profile">
            <div class="view-avatar">
              <img v-if="selectedDoctor.photo_url" :src="'https://carequeue-admin.com/api' + selectedDoctor.photo_url" alt="Profile" />
        ...
        <div class="modal-body" v-if="accountTargetDoctor">
          <div class="account-summary">
            <div class="account-summary-avatar">
              <img v-if="accountTargetDoctor.photo_url" :src="'https://carequeue-admin.com/api' + accountTargetDoctor.photo_url" alt="Profile"/>
          ...
          <div class="regen-group">
            <button class="btn-regenerate" @click="regenerateEmail">
            ...
            <button class="btn-regenerate" @click="generatedPassword = randomPassword()">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
              Regenerate Password
            </button>
          </div>
  ...
  methods: {
    ...
    async addDoctor() {
      ...
      try {
        ...
        const res = await fetch('https://carequeue-admin.com/api/doctors', { method: 'POST', body: formData })
        const data = await res.json()
    ...
    generateCredentials() {
      ...
      this.generatedUsername = this.generateEmail()
    ...
    regenerateEmail() {
      ...
      this.generatedUsername = `${first}.${last}${suffix}@cqueue.com`
    },
    ...
    async saveSpecialization() {
      ...
      try {
        const res = await fetch('https://carequeue-admin.com/api/specializations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: this.newSpecialization.trim() }) })
        const data = await res.json()
    ...
    async saveHospital() {
      ...
      try {
        const res = await fetch('https://carequeue-admin.com/api/hospitals', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: this.newHospital.trim() }) })
        const data = await res.json()
        if (!res.ok) throw new Error(data.message || 'Failed.')
    ...
    async saveOffice() {
      ...
      try {
        const res = await fetch('https://carequeue-admin.com/api/offices', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    async fetchOffices() {
      try { this.officeList = await (await fetch('https://carequeue-admin.com/api/offices')).json() }
    ...
    async submitImport() {
      ...
      try {
        ...
        const res = await fetch('https://carequeue-admin.com/api/doctors/import', { method: 'POST', body: formData })
        const data = await res.json()
    ...
    async fetchDoctors() {
      this.loading = true
      try {
        const res = await fetch('https://carequeue-admin.com/api/doctors')
    ...
    async fetchHospitals() {
      try { this.hospitalList = await (await fetch('https://carequeue-admin.com/api/hospitals')).json() }
      catch (err) { console.error('Failed to fetch hospitals:', err) }
    },

    async fetchSpecializations() {
      try { this.specializationList = await (await fetch('https://carequeue-admin.com/api/specializations')).json() }
...
.regen-group { display: flex; gap: 8px; flex-wrap: wrap; }
